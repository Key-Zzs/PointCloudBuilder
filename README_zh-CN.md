...
当前实现的是第一阶段：raw RGB-D 反投影；同时提供离线工具，用于从本机
...
- 提供独立的裁剪和固定点数采样工具，支持 `stride`、`random`、`voxel`、
  `fps`、`voxel_random`、`voxel_fps`，用于离线测试和后续 pipeline 阶段。

当前高层 `PointCloudBuilder` 输出仍是 raw 点云。裁剪和采样模块已经作为扩展点
存在，并可独立 benchmark，但还不是高层 builder 的默认输出。
